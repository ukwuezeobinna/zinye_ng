...
Used for B2C real-time receipt submission (POS fiscalization).
Docs: https://atrs.firs.gov.ng/docs/

Dev:  https://api-dev.i-fis.com
Prod: https://atrs-api.firs.gov.ng

Auth: OAuth 2.0 password grant → Bearer token (24h TTL).
Security: MD5(secret + vat_number + business_place + business_device
               + bill_number + bill_datetime + total_value)
"""
...
_DEV_BASE = "https://api-dev.i-fis.com"
_PROD_BASE = "https://atrs-api.firs.gov.ng"
...
def _base_url(settings) -> str:
    return _PROD_BASE if settings.environment == "Production" else _DEV_BASE
...
def _security_code(settings, bill_number: int, bill_datetime: str, total_value: float) -> str:
    ...
    raw = (
        secret
        + settings.vat_number
        + settings.business_place
        + settings.business_device
        + str(bill_number)
        + bill_datetime
        + str(total_value)
    )
...
def submit_receipt(
    ...
    *,
    ...
    payment_type: C=cash, T=transfer, K=credit card, D=debit card,
                  P=post payment, O=other

    Returns the ATRS response dict (includes payment_code / UID).
    Raises ATRSError on API or validation failure.
    """
    ...
    payload = {
        ...
        "business_place": settings.business_place,
        "business_device": settings.business_device,
...
def submit_pos_invoice_to_atrs(pos_invoice: str) -> dict:
    ...
    _log_atrs_submission(doc, result)
    return result
